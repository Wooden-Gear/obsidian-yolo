...
感谢你愿意为项目出力。本文档说明我们欢迎什么样的贡献、PR 怎么提，以及为了让 review 不至于失控而存在的几条简单规则。
...
- **可以用 AI 写代码。** 但你得清楚自己改了什么、为什么这么改、可能坏在哪 —— 足以在 review 时自己解释清楚。
...
- README Roadmap 上的项目（后台 Agent、Cron 定时任务、多 Agent 编排、Learning Mode、AI 白板）
...
- 你应该能用自己的话讲清楚：改了什么、为什么这么改、可能在哪坏。不需要在 review 过程中现场再问一次 LLM。
- 当 reviewer 问"这里为什么这么写？"、"这个分支处理什么情况？"、"X 边界考虑过吗？"时，你应该有答案。
- AI 有时候写得比人还好，那很正常。但 AI 也会自信地写出一些在本仓库里完全说不通的代码 —— PR 作者的责任就是在提交之前把后一种情况筛掉。

PR 模板会让你勾选是否使用了 AI。**这是知情声明，不是过滤器** —— 如实勾就行。勾了"使用 AI"的 PR 不会被区别对待；任何说不清自己改了什么的 PR，无论谁写的，一样会被关闭。
...
- Review 反馈通常比较直接。看不懂就问，欢迎短问题，不欢迎长篇辩护。
- 如果 PR 卡在等作者回复超过两周，可能会被直接关掉。准备好继续推进时随时 reopen。
